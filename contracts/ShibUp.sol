    ...
    DividendPayingTokenOptionalInterface
{
    ...
    address public immutable SHIB =
        address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // SHIB
...
contract ShibUp is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShibUpDividendTracker public dividendTracker;

    address public deadWallet = address(0xdead);

    // Mainnet
    address public immutable SHIB =
        address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // Shiba Inu

    uint256 public swapTokensAtAmount = 200000 * (10**10);

    uint256 public SHIBRewardsFee = 6;
    uint256 public liquidityFee = 1;
    uint256 public buyBackAndBurnFee = 1;
    uint256 public marketingFee = 2;

    uint256 public totalFees =
        SHIBRewardsFee.add(liquidityFee).add(buyBackAndBurnFee).add(
            marketingFee
        );

    address public _liquidityWallet =
        0xEab53542a1Df0eB6aED363E2B9d2F3C311CA0398;
    address public _marketingWallet =
        0x5b0231F2243A98137855716c679B41bda227aFEA;
    ...
    constructor() public ERC20("ShibUp", "SUT") {
        dividendTracker = new ShibUpDividendTracker();
    ...
    function swapOnDemand(bool liquify, bool sendDividends) external onlyOwner {
        swapping = true;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 swapMarketingTokens = contractTokenBalance
                .mul(marketingFee)
                .div(totalFees);
            swapTokensForEth(swapMarketingTokens);
            uint256 marketingBnb = address(this).balance;

            uint256 swapBuyBackAndBurnTokens = contractTokenBalance
                .mul(buyBackAndBurnFee)
                .div(totalFees);
            swapTokensForEth(swapBuyBackAndBurnTokens);
            uint256 buyBackBalanceBnb = address(this).balance.sub(marketingBnb);

            uint256 sellTokens = balanceOf(address(this));
            swapAndSendDividends(sellTokens);

            (bool success, ) = payable(_marketingWallet).call{
                value: marketingBnb
            }("");
            require(success, "can't send marketing fee");
...
contract ShibUpDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        public
        DividendPayingToken("ShibUp_Dividen_Tracker", "ShibUp_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**10); // must hold 1000+ tokens
    }
    ...
    function updateMinimumTokenBalanceForDividends(uint256 _newMinimumBalance)
        ...
        onlyOwner
    {
        minimumTokenBalanceForDividends = _newMinimumBalance * (10**10);
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        ...
        require(newClaimWait != claimWait, "same value");
